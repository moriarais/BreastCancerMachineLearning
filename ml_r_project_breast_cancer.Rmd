...
### Subtype
The accuracy value we recieved is very low. no metter the trial number or the model it didnt improve. we will count on the other algorithms
```{r}
...
print(paste("Accuracy:", round(acc * 100, 2), "%"))
```
As seen, the estrogen and progesteron receptors have the best accuracy so far. her2, with a small data, dosn't show the best results. and subtype- our main goul in this project show accuracy over 80%. we will go through a few more ml algorithms to try and perfect as we can the accuracy, with foucos on subtype.
...
The data we used is quite heavy, therefore, we are trying to create a smaller version for faster analysis. 
```{r}
