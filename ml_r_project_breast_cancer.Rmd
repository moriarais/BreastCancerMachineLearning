...
rf_fit <- ranger(train_labels_her2 ~ ., 
                 data = train_data, 
                 num.trees = 2000,
                 max.depth = 20,
                 probability = TRUE)
...
rf_fit <- ranger(train_labels_her2 ~ ., 
                 data = train_data, 
                 num.trees = 6000,
                 mtry = 0.2,       # Specify a fraction of predictors to consider at each split
                 min.node.size = 30, # Increase this for larger data
                 sample.fraction = 0.5,  # Use a fraction of your data for each tree
                 max.depth = 100,
                 probability = TRUE)
...
```{r}
model_dt_her2 <- C5.0(train_data, train_labels_her2, trials = 10, model = "C5.0Rules")
